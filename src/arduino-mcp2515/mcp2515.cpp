...
uint8_t MCP2515::readRegister(const REGISTER reg) {
    startSPI();
    spi_out(INSTRUCTION_READ);
    spi_out(reg);
    uint8_t ret = spi_out(0x00);
    endSPI();
...
uint8_t MCP2515::getStatus(void) {
    startSPI();
    spi_out(INSTRUCTION_READ_STATUS);
    uint8_t i = spi_out(0x00);
    endSPI();
...
MCP2515::ERROR MCP2515::setMode(const CANCTRL_REQOP_MODE mode) {
    ...
    unsigned long endTime   = systick_get_ms() + 10;
    bool          modeMatch = false;
    while (systick_get_ms() < endTime) {
